 ...
 * @returns 是否匹配
 */
export function isRegionMatch(region: string, searchTerm: string): boolean {
  ...
  // CFSM 兼容：region 可能是 ISO 代碼或中文，先解析為映射條目
  const regionInfo = emojiToRegionMap[region] ?? resolveRegionInfo(region)
  if (!regionInfo) {
    // 如果映射表中没有，则只进行简单的包含匹配
    return region.toLowerCase().includes(lowerSearchTerm)
  }
 ...
 * @returns 地区名称
 */
export function getRegionDisplayName(region: string, language: 'en' | 'zh' = 'zh'): string {
  const regionInfo = emojiToRegionMap[region] ?? resolveRegionInfo(region)
  if (!regionInfo) {
    return region
  }
...
export function getRegionCode(region: string): string {
  ...
  if (!regionInfo) {
    ...
    return upper.length === 2 ? upper : 'UN'
  }
...
/**
 * CFSM 兼容輔助：把 ISO 代碼 / 中文名 / emoji 解析為映射條目
 * @param input 地區標識
 * @returns 地區信息，未找到返回 null
 */
function resolveRegionInfo(input: string): { en: string, zh: string, code: string, aliases: string[] } | null {
  if (!input)
    return null
  ...
  // 直接匹配 emoji
  if (emojiToRegionMap[input])
    return emojiToRegionMap[input]

  for (const info of Object.values(emojiToRegionMap)) {
    if (info.code.toLowerCase() === lowerInput)
      return info
    if (info.zh === input.trim())
      return info
